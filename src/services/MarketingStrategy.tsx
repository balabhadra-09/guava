...
export function MarketingStrategy() {
  return (
    <>
      <Helmet>
        <title>Marketing Strategy | Synapse</title>
        <meta name="description" content="Expert Marketing Strategy services by Synapse Digital Agency." />
      </Helmet>
    ...
    <div className="absolute inset-0 bg-gradient-to-br from-cyan-400/20 via-blue-500/20 to-transparent opacity-30" />
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[40rem] h-[40rem] bg-cyan/10 rounded-full blur-[120px] pointer-events-none" />
    <div className="container relative z-10 px-6 mx-auto text-center pt-32 pb-20 lg:pt-48 lg:pb-32">
      <h1 className="text-5xl lg:text-7xl font-bold mb-6 font-outfit">Marketing Strategy</h1>
      <p className="text-xl text-gray-400 max-w-2xl mx-auto mb-10">Premium Marketing Strategy solutions for forward-thinking brands.</p>
    </div>
        ...
        <SectionWrapper>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[1, 2, 3, 4].map(i => (
              <Card key={i} className="p-8 flex items-start gap-6 border-l-4 border-l-rose-500">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between relative">
            ...
            {['Discovery', 'Strategy', 'Execution', 'Scale'].map((step, i) => (
              <div key={step} className="relative z-10 glass p-6 rounded-2xl w-full md:w-1/4 text-center">
                <div className="w-8 h-8 rounded-full bg-rose-500 text-dark-900 font-bold mx-auto mb-4 flex items-center justify-center">{i+1}</div>
