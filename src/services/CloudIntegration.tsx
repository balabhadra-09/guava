...
export function CloudIntegration() {
  return (
    <>
      <Helmet>
        <title>Cloud Integration | Synapse</title>
        <meta name="description" content="Expert Cloud Integration services by Synapse Digital Agency." />
      </Helmet>
    ...
    <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-indigo-500/20 via-purple-500/20 to-transparent" />
    <div className="container relative z-10 px-6 mx-auto pt-40 pb-20 lg:pt-56 lg:pb-32">
      <div className="max-w-3xl">
        <h1 className="text-6xl lg:text-8xl font-bold mb-8 font-outfit tracking-tighter">Cloud Integration</h1>
        <p className="text-2xl text-gray-500 mb-10 font-light">Premium Cloud Integration solutions for forward-thinking brands.</p>
      </div>
    </div>
        ...
        <SectionWrapper>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[1, 2, 3, 4].map(i => (
              <Card key={i} className="p-8 flex items-start gap-6 border-l-4 border-l-indigo-400">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between relative">
            ...
            {['Discovery', 'Strategy', 'Execution', 'Scale'].map((step, i) => (
              <div key={step} className="relative z-10 glass p-6 rounded-2xl w-full md:w-1/4 text-center">
                <div className="w-8 h-8 rounded-full bg-indigo-400 text-dark-900 font-bold mx-auto mb-4 flex items-center justify-center">{i+1}</div>
