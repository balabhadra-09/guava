...
export function VideoProduction() {
  return (
    <>
      <Helmet>
        <title>Video Production | Synapse</title>
        <meta name="description" content="Expert Video Production services by Synapse Digital Agency." />
      </Helmet>
    ...
    <div className="absolute inset-0 bg-gradient-to-r from-fuchsia-500/20 via-rose-500/20 to-transparent opacity-20" />
    <div className="container relative z-10 px-6 mx-auto pt-32 pb-20 lg:pt-48 lg:pb-32 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
      <div>
        <h1 className="text-5xl lg:text-7xl font-bold mb-6 font-outfit">Video Production</h1>
        <p className="text-xl text-gray-400 mb-10">Premium Video Production solutions for forward-thinking brands.</p>
      </div>
      <div className="relative aspect-square">
        <div className="absolute inset-0 bg-gradient-to-tr from-fuchsia-500/20 via-rose-500/20 to-transparent rounded-full blur-3xl opacity-40 animate-pulse" />
        ...
        <SectionWrapper>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[1, 2, 3, 4].map(i => (
              <Card key={i} className="p-8 flex items-start gap-6 border-l-4 border-l-emerald-400">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between relative">
            ...
            {['Discovery', 'Strategy', 'Execution', 'Scale'].map((step, i) => (
              <div key={step} className="relative z-10 glass p-6 rounded-2xl w-full md:w-1/4 text-center">
                <div className="w-8 h-8 rounded-full bg-emerald-400 text-dark-900 font-bold mx-auto mb-4 flex items-center justify-center">{i+1}</div>
